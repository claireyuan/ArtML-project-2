...
if use_existing_model:
    netDB = load_model("models/netDB")
    netDA = load_model("models/netDA")
else:
...
if use_existing_model:
    netGB = load_model("models/netGB")
    netGA = load_model("models/netGA")
else:
...
train_A = load_data('datasets/fish_data/*.jpg')
train_B = load_data('datasets/flowers_data/*.jpg')
print(len(train_B))
...
def showX(X, rows=1, counter=-1):
    ...
    display(img)
    img.save("results/res" + str(counter) + ".jpg")
...
while epoch < niter: 
    ...
    if gen_iterations%display_iters==0:
        ...
        save_model(netDA, 'models/' + 'netDA')
        save_model(netDB, 'models/' + 'netDB')
        save_model(netGA, 'models/' + 'netGA')
        save_model(netGB, 'models/' + 'netGB')
